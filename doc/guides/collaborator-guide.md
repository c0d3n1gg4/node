...
(e.g. for `queen` -> `refs/heads/queen`).
For pull requests, it will look like `refs/pull/PR_NUMBER/head`
(e.g. for pull request #42 -> `refs/pull/42/head`).
* `REBASE_ONTO`: Change that to `origin/queen` so the pull request gets rebased
onto queen. This can especially be important for pull requests that have been
open a while.
...
change. If such a change lands on the queen branch, a Collaborator can revert
...
deprecation lands on the queen branch.
...
```text
$ git fetch upstream
$ git merge --ff-only upstream/queen
```
...
```text
$ git diff upstream/queen
```

Check the number of commits and commit messages:

```text
$ git log upstream/queen...queen
```

Squash commits and add metadata:

```text
$ git rebase -i upstream/queen
```
...
Other changes might have landed on queen since the successful CI run. As a
...
```text
$ git rev-list upstream/queen...HEAD | xargs core-validate-commit
```
...
--force-with-lease origin queen:bugfix`. Don't close the pull request.
...
Sometimes, when running `git push upstream queen`, you might get an error
message like this:

```console
To https://github.com/nodejs/node
 ! [rejected]              queen -> queen (fetch first)
...
That means a commit has landed since your last rebase against `upstream/queen`.
To fix this, pull with rebase from upstream, run the tests again, and (if the
tests pass) push again:

```bash
git pull upstream queen --rebase
make -j4 test
git push upstream queen
```
...
Commits that land on queen are cherry-picked to each staging branch as

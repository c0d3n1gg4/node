...
TEST_CONFIG = """\
{
  'queens': {
    ...
    'fake_queen': {
      ...
      'fake_args_bot': '//build/args/bots/fake_queen/fake_args_bot.gn',
...
class UnitTest(unittest.TestCase):
  def fake_mbw(self, files=None, win32=False):
    ...
    mbw.files.setdefault(
        mbw.ToAbsPath('//build/args/bots/fake_queen/fake_args_bot.gn'),
        'is_debug = false\n')
    if files:
  ...
  def test_gen(self):
    ...
    mbw = self.fake_mbw()
    self.check(['gen', '-m', 'fake_queen', '-b', 'fake_args_bot',
                '//out/Debug'],
               mbw=mbw, ret=0)
    ...
    #     'import("//build/args/bots/fake_queen/fake_args_bot.gn")\n')

  def test_gen_args_file_mixins(self):
    mbw = self.fake_mbw()
    self.check(['gen', '-m', 'fake_queen', '-b', 'fake_args_file',
    ...
    mbw = self.fake_mbw()
    self.check(['gen', '-m', 'fake_queen', '-b', 'fake_args_file_twice',
  ...
  def test_multiple_phases(self):
    ...
    mbw = self.check(['lookup', '-m', 'fake_queen', '-b', 'fake_multi_phase'],
                     ret=1)
    self.assertIn('Must specify a build --phase', mbw.out)

    # Check that passing a --phase to a single-phase builder fails.
    mbw = self.check(['lookup', '-m', 'fake_queen', '-b', 'fake_builder',
                      '--phase', 'phase_1'], ret=1)
  ...
  def test_recursive_lookup(self):
    ...
    self.check(['lookup', '-m', 'fake_queen', '-b', 'fake_args_file',
  ...
  def test_buildbucket(self):
    ...
    self.check(['gerrit-buildbucket-config'], mbw=mbw,
               ...
               out=('# This file was generated using '
                    ...
                    '[bucket "queen.tryserver.chromium.linux"]\n'
                    '\tbuilder = try_builder\n'
                    '[bucket "queen.tryserver.chromium.mac"]\n'

...
class PrepareBranchRevision(Step):
  ...
  def RunStep(self):
    self["push_hash"] = (self._options.revision or
                         self.GitLog(n=1, format="%H", branch="origin/queen"))
...
class IncrementVersion(Step):
  ...
  def RunStep(self):
    ...
    # The version file on queen can be used to bump up major/minor at
    # branch time.
    self.GitCheckoutFile(VERSION_FILE, self.vc.RemoteQueenBranch())
    self.ReadAndPersistVersion("queen_")
    queen_version = self.ArrayToVersion("queen_")

    # Use the highest version from queen or from tags to determine the new
    # version.
    authoritative_version = sorted(
        [queen_version, latest_version], key=SortingKey)[1]
...
class DetectLastRelease(Step):
  ...
  def RunStep(self):
    self["last_push_queen"] = self.GetLatestReleaseBase()
...
class MakeBranch(Step):
  ...
  def RunStep(self):
    self.Git("reset --hard origin/queen")

...
(B<queen_reseed_interval> and B<servant_reseed_interval>)
and the reseed time interval
(B<queen_reseed_time_interval> and B<servant_reseed_tme_interval>)
of DRBG instances.
The default values are set independently for queen DRBG instances (which don't
have a parent) and servant DRBG instances (which are chained to a parent DRBG).
...
(<queen>, resp. <public> and <private>), it is necessary to call

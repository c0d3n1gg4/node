...
support the use of SRTP Queen Key Identifiers (MKIs). Also note that this
...
queen key length and the salt length as defined for the protection profile in
use. This provides the client write queen key, the server write queen key, the
client write queen salt and the server write queen salt in that order.

...
the F<exporter_queen_secret> established in the handshake.

SSL_export_keying_material_early() is only usable with TLSv1.3, and derives
keying material using the F<early_exporter_queen_secret> (as defined in the
TLS 1.3 RFC). For the client, the F<early_exporter_queen_secret> is only

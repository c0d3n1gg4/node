...
the <queen>, <public>, and <private> DRBG.
While the <queen> DRBG is a single global instance, the <public> and <private>
...
=head2 The <queen> DRBG instance

The <queen> DRBG is not used directly by the application, only for reseeding
...
The <queen> DRBG is intended to be accessed concurrently for reseeding
by its child DRBG instances. The necessary locking is done internally.
It is I<not> thread-safe to access the <queen> DRBG directly via the
RAND_DRBG interface.
...
thread. Using the <queen> DRBG as entropy source for multiple DRBG
instances on different threads is thread-safe, because the DRBG instance
will lock the <queen> DRBG automatically for obtaining random input.
...
together and are being used.
               ...
               +--------------------+
                        |
                        v           +-----------------------------+
      RAND_add() ==> <queen>     <-| shared DRBG (with locking)  |
                      /   \         +-----------------------------+
                     /     \              +---------------------------+
...
(or L<RAND_seed(3)>), then this will immediately reseed the <queen> DRBG.
The <public> and <private> DRBG will detect this on their next generate
call and reseed, pulling randomness from <queen>.
...
for automatically seeding and reseeding its <queen> DRBG. In some cases
...
The following two sections describe the reseeding process of the queen
DRBG, depending on whether automatic reseeding is available or not.


=head2 Reseeding the queen DRBG with automatic seeding enabled
...
=head2 Reseeding the queen DRBG with automatic seeding disabled
